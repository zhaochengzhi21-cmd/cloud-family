...
export default function FamilyPage() {
  ...
  // 我有兴趣 / 连接状态
  const [interestState, setInterestState] = useState<Record<string, { iAmInterested: boolean; theyAreInterested: boolean; mutual: boolean }>>({});
  const [interestSending, setInterestSending] = useState<string | null>(null);
  const [activeConnection, setActiveConnection] = useState<{ connectedFamilyId: string; connectedFamilyName: string; connectedAt: string } | null>(null);
              ...
              {matchingResults && matchingResults.length > 0 && (
                <div className="mt-4 space-y-3">
                  {matchingResults.map((match: any, idx: number) => {
                    const interested = interestState[match.familyId] || { iAmInterested: false, theyAreInterested: false, mutual: false };
                    return (
                      <div
                        key={match.familyId}
                      ...
                      >
                        ...
                        <div className="mt-3 flex items-center gap-2">
                          {interested.mutual ? (
                            ...
                            <span className="inline-flex items-center gap-1 px-3 py-1.5 bg-amber-100 text-amber-700 rounded-lg text-xs font-bold">
                              ⏳ 已发送意向，等待对方回应
                            </span>
                          ) : (
                            <button
                              disabled={interestSending === match.familyId}
                              onClick={async () => {
                                setInterestSending(match.familyId);
                                try {
                                  const res = await fetch(`/api/family-matching/${familyId}/interest`, {
                                    method: "POST",
                                    headers: { "Content-Type": "application/json" },
                                    body: JSON.stringify({
                                      targetFamilyId: match.familyId,
                                      action: "express",
                                      targetFamilyName: match.familyName,
                                    }),
                                  });
                                  const data = await res.json();
                                  if (data.success) {
                                    setInterestState(prev => ({
                                      ...
                                      [match.familyId]: {
                                        iAmInterested: true,
                                        theyAreInterested: data.mutual || false,
                                        mutual: data.mutual || false,
                                      },
                                    }));
                                    if (data.mutual && data.connection) {
                                      // 建立连接
                                      setActiveConnection({
                                        connectedFamilyId: match.familyId,
                                        connectedFamilyName: match.familyName,
                                        connectedAt: data.connection.connectedAt || new Date().toISOString(),
                                      });
                                    }
                                  } else {
                                    // 显示错误
                                    console.error("兴趣请求失败:", data.error);
                                    // 重置兴趣状态
                                    setInterestState(prev => {
                                      const next = { ...prev };
                                      delete next[match.familyId];
                                      return next;
                                    });
                                  }
                                } catch (err) {
                                  console.error("兴趣请求异常:", err);
                                  // 重置兴趣状态
                                  setInterestState(prev => {
                                    const next = { ...prev };
                                    delete next[match.familyId];
                                  ...
                                  setInterestSending(null);
                                }
                              }}
                              className="px-3 py-1.5 bg-[#8b0000] text-white rounded-lg text-xs font-bold hover:bg-[#a52a2a] disabled:opacity-40 transition-colors"
                            >
                              {interestSending === match.familyId ? (
                                <span className="inline-flex items-center gap-1">
                                  <div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                                  发送中...
                                </span>
                  ...
                  <AnonymousChat
                    ...
                    onDisconnected={() => {
                      ...
                      setInterestState(prev => {
                        const next = { ...prev };
                        if (activeConnection) {
                          delete next[activeConnection.connectedFamilyId];
                        }
                        return next;
                      });
                    }}
